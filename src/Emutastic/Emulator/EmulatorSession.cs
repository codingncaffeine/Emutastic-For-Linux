...
namespace Emutastic.Emulator
{
    ...
    public sealed class EmulatorSession : IDisposable
    {
        ...
        public EmulatorSession(string corePath, string romPath, string console = "", Models.Game? game = null)
        {
            ...
            // installed. (Upstream seeds these in SeedDefaultCoreOptions.)
            if (_game != null)
        ...
        public bool Start(out string? error)
        {
            ...
            try
            {
                ...
                // Saves are organized per-console (Saves/<Console>/) — the Linux analog of
                // upstream's BatterySaves/<Console>/ tree. This keeps each core's memory cards
                // and save trees (PSP/, User/ for Dolphin, dc/, Azahar/, …) under their own
                // console folder, so cloud sync can attribute them by console (see
                // GitHubSyncService.BuildExtraSaveMap) and the folder stays organized.
        ...
        private void RunLoop()
        {
            ...
            while (_running)
            {
                ...
                // The achievement runtime resets with the game: rcheevos requires rc_client_reset on
                // every game reset so hit counts and active leaderboards don't survive it.
        ...
        /// <summary>rc_client_reset alongside retro_reset — see the emu loop.</summary>
