...
namespace Emutastic.Services
{
    ...
    public sealed class GitHubSyncService
    {
        ...
        // <RomHash>.srm) so one repo serves Windows and Linux installs alike.

        public sealed record LocalSave(string RepoPath, string LocalPath, DateTime LastModifiedUtc, long SizeBytes);

        public static string LocalSrmPathFor(string romPath)
        ...
        public async Task<SyncResult> FullSyncAsync(DatabaseService db, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // DOWNLOAD: remote saves newer than local (or missing locally).
                ...
                foreach (var (repoPath, entry) in _manifestCache.Files)
                {
                    ...
                    bool shouldDownload = !File.Exists(targetPath)
                        || (DateTime.TryParse(entry.LastModifiedUtc, null,
                                System.Globalization.DateTimeStyles.RoundtripKind, out var remoteMtime)
                            && remoteMtime > File.GetLastWriteTimeUtc(targetPath));
        ...
        public async Task PullSaveBeforeLaunchAsync(Models.Game game, CancellationToken ct = default)
        {
            ...
            {
                Trace.WriteLine("[CloudSync] Lock timeout — skipping download, using local save");
                return;
            }
            try
            {
                ...
                if (File.Exists(localPath)
                    && _manifestCache.Files.TryGetValue(repoPath, out var mEntry)
                    && DateTime.TryParse(mEntry.LastModifiedUtc, null,
                        System.Globalization.DateTimeStyles.RoundtripKind, out var remoteMtime))
                    shouldDownload = remoteMtime > File.GetLastWriteTimeUtc(localPath);
                ...
                if (remote != null && remote.Length > 0)
                {
                    ...
                    Trace.WriteLine($"[CloudSync] Downloaded remote save: {repoPath}");
                }
            }
            catch (Exception ex)
            {
                Trace.WriteLine($"[CloudSync] Pre-launch download failed: {ex.Message}");
            }
        ...
        public async Task UploadSaveAfterSessionAsync(Models.Game game, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (await UploadFileAsync(repoPath, srmBytes, ct).ConfigureAwait(false))
                {
                    ...
                    await SaveManifestAsync(ct).ConfigureAwait(false);
                    Trace.WriteLine($"[CloudSync] Uploaded save: {repoPath}");
                }
            }
            catch (Exception ex)
            {
                Trace.WriteLine($"[CloudSync] Post-session upload failed: {ex.Message}");
            }
        }
    }
}
